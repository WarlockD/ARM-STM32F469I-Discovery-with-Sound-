...
static boolean ReadTrackHeader(midi_track_t *track, FILE *stream)
{
    ...
    track->data_len = __REV(chunk_header.chunk_size);
...
static boolean ReadFileHeader(midi_file_t *file, FILE *stream)
{
    ...
    if (!CheckChunkHeader(&file->header.chunk_header, HEADER_CHUNK_ID)
     || __REV(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
						__REV(file->header.chunk_header.chunk_size));
        return false;
    }

    format_type = __REV16(file->header.format_type);
    file->num_tracks = __REV16(file->header.num_tracks);
...
{
    short result = __REV16(file->header.time_division);
